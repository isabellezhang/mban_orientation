...
jp_best2 <- select(arrange(filter(listings,neighbourhood == "Jamaica Plain"), desc(review_scores_rating)), neighbourhood, name, review_scores_rating)
...
# x %>% f(y) <=> f(x,y) take x and do f with parameter y
...
listings %>% 
  ...
  summarize(price_per = mean(price_per),
            n = n(), 
            mean_rating = mean(review_scores_rating, na.rm = TRUE)) # n gives how many listing we have 
...
september_availability <- calendar %>% 
  ...
  filter(month == 'Sep' ) %>% 
  group_by(listing_id) %>% 
  summarize(nights_available = sum(available)) %>% # no need to do available == TRUE, assumed for logical variable
...
listings <- listings %>% 
  left_join(september_availability,
            by = c('id' = 'listing_id')) # id column in listing table and listing id column in calendar table
...
summarytable %>% 
  ...
  geom_bar(stat = 'identity') + # stat = 'identify' allows us to specify the y label
  #geom_point()
  coord_flip() +
  # xlab('Neighbourhood') +
  # ylab('Number of Apartments')
  labs(x = 'Neighbourhoods', y = 'Number of Apartments', title = 'My Bar Chart', subtitle = 'Never do pie charts')
...
calendar %>% 
  ...
  summarize(avg_price = mean(price, na.rm = TRUE)) %>%  # till here gives you dataset with 3 cols
  ggplot() +
  aes(x = date, y = avg_price, color = is_boat) +
  geom_line() +
  facet_wrap(~is_boat)
